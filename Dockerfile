...
# 安装模块
COPY ./base/modules.conf /opt/freeswitch-${VERSION}.-release/modules.conf
...
# 添加LuaJIT并安装
ADD ./base/LuaJIT-2.0.5.tar.gz /opt/
RUN cd /opt/LuaJIT-2.0.5/ && make -j4 && make install && rm -rf /opt/LuaJIT-2.0.5

# 安装lua-cjson
ADD ./base/lua-cjson-2.1.0.tar.gz /opt/
...
COPY ./base/freeswitch.limits.conf /etc/security/limits.d/
